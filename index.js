...
var handleGlue = function (event, context) {
  var subject = 'AWS Glue Notification'
  var timestamp = new Date(event.Records[0].Sns.Timestamp).getTime() / 1000
  var message
  var fields = []
  var color = 'warning'
  ...
  try {
    ...
    header = message.detail.state + ': Glue'
    fields.push({ title: 'Message', value: header, short: true })
    fields.push({ title: 'Severity', value: message.detail.severity, short: true })
    fields.push({ title: 'Job Name', value: message.detail.jobName, short: false })
    fields.push({ title: 'Job Run Id', value: message.detail.jobRunId, short: false })
    fields.push({ title: 'Region', value: message.region, short: true })
    fields.push({ title: 'Details', value: message.detail.message, short: false })
  } catch (e) {
    color = 'warning'
    message = event.Records[0].Sns.Message
    header = message.detail.state + ': Glue '
...
var processEvent = function (event, context) {
  ...
  if (
    ...
    eventSubscriptionArn.indexOf(config.services.glue.match_text) > -1 ||
    eventSnsSubject.indexOf(config.services.glue.match_text) > -1 ||
    eventSnsMessageRaw.indexOf(config.services.glue.match_text) > -1
  ) {
    console.log('processing glue notification')
    slackMessage = handleGlue(event, context)
  } else {
